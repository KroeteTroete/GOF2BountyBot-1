...
async def cmd_kaamo_get(message : discord.Message, args : str, isDM : bool):
    ...
    if requestedBBUser.kaamo is None or requestedBBUser.kaamo.totalItems == 0:
        ...
        if len(argsSplit) < 2:
            await message.channel.send(":x: Not enough arguments! Please provide both an item type " \
                                        + "(ship/weapon/module/turret/tool) and an item number from `" \
                                        + commandPrefix + "kaamo`")
            return
        if len(argsSplit) > 2:
            await message.channel.send(":x: Too many arguments! Please only give an item type " \
                                        + "(ship/weapon/module/turret/tool), and an item number from `" \
                                        + commandPrefix + "kaamo`")
            return
...
bbCommands.register("kaamo", cmd_kaamo, 0, allowDM=True, helpSection="kaamo club", signatureStr="**kaamo** *[item-type]*",
                    ...
                    longHelp="List all items in your Kaamo Club storage. Kaamo has a max capacity of " \
                        ...
                        + "⚠ Please be aware that `$kaamo store` and `$kaamo get` can only be used by level 10 bounty hunters.")
