...
async def dev_cmd_clear_bounties(message : discord.Message, args : str, isDM : bool):
    ...
    if args == "all":
        ...
        if not botState.guildsDB.guildsDB.guildIdExists(int(args)):
...
async def dev_cmd_resetnewbountycool(message : discord.Message, args : str, isDM : bool):
    ...
    if guildStr == "":
        ...
        if not botState.guildsDB.guildsDB.guildIdExists(int(guildStr)):
            await message.channel.send("Unrecognised guild")
            return
...
bbCommands.register("make-bounty", dev_cmd_make_bounty, 2, forceKeepArgsCasing=True, allowDM=True, helpSection="bounties",
                    useDoc=True)
