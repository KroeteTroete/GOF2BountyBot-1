...
class UABase(ABC):
    ...
    userAlerts have a state (off/on), a toggle method (switches state), and a setState method
    ...
    async def toggle(self, dcGuild : Guild, bGuild : 'basedGuild.BasedGuild', dcMember : Member) -> bool:
        ...
        :param discord.Guild dcGuild: The guild in which to toggle the alert. Some UserAlert implementations
                                        ignore this parameter.
        :param BasedGuild bGuild: The BasedGuild corresponding to dcGuild. Parameter is marked for removal
    ...
    def getState(self, dcGuild : Guild, bGuild : 'basedGuild.BasedGuild', dcMember : Member) -> bool:
        ...
        :param discord.Guild dcGuild: The guild in which to get the alert state. Some UserAlert implementations ignore
                                        this parameter.
        :param BasedGuild bGuild: The BasedGuild corresponding to dcGuild. Parameter is marked for removal from this
                                    method's signature.
    ...
    async def setState(self, dcGuild : Guild, bGuild : 'basedGuild.BasedGuild', dcMember : Member, newState : bool) -> bool:
        ...
        :param discord.Guild dcGuild: The guild in which to set the alert state. Some UserAlert implementations ignore
                                        this parameter.
        :param BasedGuild bGuild: The BasedGuild corresponding to dcGuild. Parameter is marked for removal from this
                                    method's signature.
...
class StateUserAlert(UABase):
    """A simple toggle-switch based UserAlert implementation that does not interface with discord or the rest of BountyBot
    in any way. Essentially acts as a boolean variable. In all methods, the given guild and member are ignored.
    ...
    async def toggle(self, dcGuild : Guild, bGuild : 'basedGuild.BasedGuild', dcMember : Member) -> bool:
        ...
        :param BasedGuild bGuild: Ignored
...
class GuildRoleUserAlert(UABase):
    ...
    async def toggle(self, dcGuild : Guild, bGuild : 'basedGuild.BasedGuild', dcMember : Member) -> bool:
        ...
        if alertRole is None:
            raise ValueError("Requested guild does not have a role set for the requested alert: " \
                                + userAlertsTypesIDs[type(self)])
...
def getAlertIDFromHeirarchicalAliases(alertName : Union[str, List[str]]) -> List[str]:
    ...
    # :param alertName: A reference to an alert. Heirarchy levels can be given as a space-separated string, or ordered list
    # elements. E.g: 'bot patches major' or ['bot', 'patches', 'major']
    :type alertName: list[str] or str
    :return: A list of UserAlert IDs in accordance with userAlerts.userAlertsIDsTypes, that are associated with the requested
                alert reference.
    :rtype: list[str]
    """
    ...
    if alertName[0] in ["bounty", "bounties"]:
        ...
        if alertName[1] in ["update", "updates", "patch", "patches", "version", "versions"]:
            if len(alertName) < 3:
                return ["ERR", ":x: Please provide the type of updates pings you would like! " \
                        + "Valid types include `major` and `minor`."]
